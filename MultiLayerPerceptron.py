...
if __name__ == '__main__':
    ...
    history = model.fit(X_train, Y_train, validation_split=0.1, epochs=400, batch_size=30)
    evaluate_dict = model.evaluate(X_test, Y_test,return_dict=True)
    y_test_prediction = model.predict_classes(X_test,verbose=0)
    y_train_prediction = model.predict_classes(X_train,verbose=0)
