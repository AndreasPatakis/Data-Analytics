...
def timeseries_to_supervised(df, n_in, n_out):
    ...
    for i in range(n_in, 0, -1):
        ...
        agg = pd.concat([ag, df_shifted], axis=1)
...
model.fit(Xtr, ytr, epochs=10, batch_size=b_size, verbose=0)

trainSetPrediction = model.predict(Xtr, batch_size=b_size)
testSetPrediction = model.predict(Xte, batch_size=b_size)
...
plot.axvline(superv_ise.index[train_set_sz], c="b")
plot.show()
